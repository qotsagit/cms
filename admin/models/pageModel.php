...
class pageModel extends Model
{
    ...
    public function Lists()
    {
        ...
        if($this->id_lang == 0)
        {
            ...
            $params = array(':id_lang' => $this->id_lang,':id_parent' => $this->id_parent);
            if ($this->Limit > 0)
                $sql = 'SELECT * FROM page WHERE id_lang=:id_lang AND id_parent=:id_parent ORDER BY ' . $this->OrderFieldName . ' ' . $asc . ' LIMIT ' . $this->LimitFrom . ',' . $this->Limit . '';
            else
                $sql = 'SELECT * FROM page WHERE id_lang=:id_lang AND id_parent=:id_parent ORDER BY ' . $this->OrderFieldName . ' ' . $asc;
        }
